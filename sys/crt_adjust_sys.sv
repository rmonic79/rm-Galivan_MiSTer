...
(
    ...
    // 2026-09-08: un registro solo e' giusto SE il chiamante consegna un vb gia'
    // in ritardo di una riga sul DE — che e' il caso di Night Slashers (DE su
    // 25..264, vb dichiara 26..265) e per questo li' tornava. Seibu Cup gli da'
    // un vb allineato al DE (crt_vsize se lo rigenera dal de_in), la latenza del
    // read non e' piu' compensata e l'ULTIMA riga attiva viene scritta, riletta
    // e blankata: misurato 239 righe invece di 240 a ogni valore di V-Size,
    // compreso lo zero dove il motore del ring e' spento.
    // Qui la dipendenza dalla fase del vb esterno sparisce: la finestra di
    // lettura la ricavo dal CONTENUTO del linebuffer. Se la riga appena scritta
    // aveva pixel attivi, la riga che il read sta per rigiocare li ha — vero per
    // costruzione, con qualunque fase di vb_in.
    ...
    // ------------------------------------------------------------------
    //  Il positivo e' un ritardo assoluto e non dipende dal quadro. Il negativo
    //  invece e' un GIRO: -N = ritarda di (righe del quadro - N). Quelle righe
    //  vanno MISURATE, non prese dal parametro: con un crt_vsize a monte il
    //  quadro ha VTOTAL+vsize righe, e con la costante il primo scatto negativo
    //  sbagliava esattamente di vsize (misurato: la traversata 0 -> -1 valeva
    //  1+vsize invece di 1, mentre tutti gli altri scatti valevano 1).
    localparam integer VSHREG = VTOTAL + 32;   // quadro piu' lungo che il V-Size possa produrre

    wire vs_rise_in = pxl_cen && vs_in && !vs_in_q;
    reg        vs_seen;
    ...
    always @(posedge clk) begin
        if (bypass) begin
            ...
            if (pxl2_cen) begin
                ...
                // In ramo attivo tutto cio' che si pubblica deve descrivere il
                // flusso EMESSO, non quello in ingresso: hb_out lo fa gia' con
                // ~pass_q, vb_out pubblicava ancora il vb del chiamante e
                // annullava a valle l'ultima riga che il read stava emettendo.
                vb_out <= vb_active;
            end
        end
    end
